...
export const AppSecDashboard: React.FC = () => {
  ...
  return (
    <div className={`flex flex-col flex-1 h-full overflow-hidden ${isLight ? 'bg-slate-100/80 text-slate-800' : 'bg-white text-slate-100'}`}>
      ...
      <div className={`px-4 py-2.5 border-b flex flex-col gap-2.5 shrink-0 ${isLight ? 'bg-slate-100/90 border-slate-200/90 shadow-xs' : 'bg-white/90 border-slate-200'}`}>
            ...
            <div>
              ...
              <div className="flex items-center gap-3 text-xs text-slate-600 mt-0.5">
                <span className="flex items-center gap-1 font-mono text-[11px] text-slate-800">
                  ...
                  {currentProject.name} ({currentProject.repoUrl}) : <span className="text-indigo-300">{currentProject.branch}</span>
                </span>
                <span>•</span>
        ...
        {/* Project Selector & Scan Action (Moved below Enterprise Checkouts & Payment service) */}
        <div className="flex items-center justify-between pt-1 border-t border-slate-200/60">
          <div className="flex items-center gap-2 text-xs font-medium text-slate-600">
            <span>Selected Target Project:</span>
            <select
              ...
              className="bg-white border border-slate-300 text-xs text-slate-900 rounded-lg px-3 py-1.5 focus:outline-none focus:border-indigo-500 cursor-pointer font-semibold shadow-2xs"
            >
              {projects.map((p) => (
