...
export function UebaAnalyticsView() {
  ...
  const [entityFilter, setEntityFilter] = useState<'ALL' | 'EMPLOYEE' | 'CONTRACTOR' | 'PRIVILEGED_ADMIN' | 'SERVICE_ACCOUNT'>('ALL');
    ...
    <div className={`flex flex-col flex-1 h-full overflow-y-auto ${isLight ? 'bg-slate-100/80 text-slate-800' : 'bg-slate-900 text-slate-100'}`}>
      ...
      <div className={`p-4 border-b flex flex-wrap items-center justify-between gap-4 ${isLight ? 'bg-slate-100/90 border-slate-200/90 shadow-xs' : 'bg-slate-900 border-slate-800'}`}>
        ...
        <div className="flex items-center gap-2">
          ...
          {(['ALL', 'EMPLOYEE', 'CONTRACTOR', 'PRIVILEGED_ADMIN', 'SERVICE_ACCOUNT'] as const).map((cat) => (
            <button
              key={cat}
            ...
            >
              {cat === 'ALL' ? 'All' : cat === 'EMPLOYEE' ? 'Employee' : cat === 'CONTRACTOR' ? 'Contractor' : cat === 'PRIVILEGED_ADMIN' ? 'PRIVILEGED Admin' : 'SERVICE Account'}
            </button>
          ))}
        </div>
      </div>

      <div className="p-3 space-y-3 max-w-[1600px] mx-auto w-full text-xs">
        {/* KPI Metrics */}
        <div className="grid grid-cols-2 md:grid-cols-4 gap-2.5">
          <div className={`p-2.5 rounded-lg border ${isLight ? 'bg-slate-100/90 border-slate-200 shadow-2xs' : 'bg-slate-900 border-slate-800'}`}>
            <span className="text-[10px] text-slate-500 font-semibold uppercase tracking-wide">Monitored User Identities</span>
            <div className="text-base font-black mt-0.5 text-slate-900 dark:text-slate-100">1,240 Users</div>
            <span className="text-[10px] text-emerald-600 font-medium">98% Normal Baseline</span>
          </div>

          <div className={`p-2.5 rounded-lg border ${isLight ? 'bg-slate-100/90 border-slate-200 shadow-2xs' : 'bg-slate-900 border-slate-800'}`}>
            <span className="text-[10px] text-slate-500 font-semibold uppercase tracking-wide">High-Risk Profiles</span>
            <div className="text-base font-black mt-0.5 text-rose-600 dark:text-rose-400">2 Critical</div>
            <span className="text-[10px] text-rose-500 font-medium">+520% Peer Baseline Spike</span>
          </div>

          <div className={`p-2.5 rounded-lg border ${isLight ? 'bg-slate-100/90 border-slate-200 shadow-2xs' : 'bg-slate-900 border-slate-800'}`}>
            <span className="text-[10px] text-slate-500 font-semibold uppercase tracking-wide">Automated Isolations</span>
            <div className="text-base font-black mt-0.5 text-amber-600 dark:text-amber-400">1 Workstation</div>
            <span className="text-[10px] text-amber-500 font-medium">CrowdStrike EDR Active</span>
          </div>

          <div className={`p-2.5 rounded-lg border ${isLight ? 'bg-slate-100/90 border-slate-200 shadow-2xs' : 'bg-slate-900 border-slate-800'}`}>
            <span className="text-[10px] text-slate-500 font-semibold uppercase tracking-wide">Service Accounts</span>
            <div className="text-base font-black mt-0.5 text-indigo-600 dark:text-indigo-400">342 SVIDs</div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-3">
          {/* Profile Table */}
          <div className={`lg:col-span-2 p-3 rounded-lg border ${isLight ? 'bg-slate-100/90 border-slate-200 shadow-2xs' : 'bg-slate-900 border-slate-800'}`}>
          ...
          <div className={`p-3 rounded-lg border flex flex-col justify-between ${isLight ? 'bg-slate-100/90 border-slate-200 shadow-2xs' : 'bg-slate-900 border-slate-800'}`}>
            <div>
              ...
              <div className="p-2.5 rounded-md border mb-2 bg-rose-500/10 border-rose-500/20 text-slate-800 dark:text-slate-200 text-xs">
              ...
              <div className="space-y-1.5 font-mono text-xs">
                ...
                <div className="flex items-center justify-between">
                  <span className="text-[10px] text-slate-500">Unresolved Alerts:</span>
                  <span className="font-bold text-amber-600">{selectedProfile.activeAlertsCount} Alerts</span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-[10px] text-slate-500">Primary Anomaly:</span>
                  <span className="font-bold text-slate-700 dark:text-slate-300 text-[10px] truncate max-w-[160px]">{selectedProfile.lastAnomalyType}</span>
                </div>
              </div>
            </div>

            <div className="pt-2.5 mt-2.5 border-t border-slate-200 dark:border-slate-800 space-y-1.5">
              <button
                onClick={() => handleIsolateEntity(selectedProfile.id)}
                className="w-full py-1.5 bg-rose-600 hover:bg-rose-700 text-white font-bold text-xs rounded-md transition-colors flex items-center justify-center gap-1.5 cursor-pointer"
              >
              ...
              <button
                ...
                className="w-full py-1.5 bg-slate-200 dark:bg-slate-800 hover:bg-slate-300 dark:hover:bg-slate-700 text-slate-800 dark:text-slate-200 font-semibold text-xs rounded-md transition-colors flex items-center justify-center gap-1.5 cursor-pointer"
              >
        ...
        <div className={`p-3 rounded-lg border ${isLight ? 'bg-slate-100/90 border-slate-200 shadow-2xs' : 'bg-slate-900 border-slate-800'}`}>
          <div className="flex items-center gap-2 mb-2">
